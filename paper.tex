...
approaches.
%Moreover the roadmap is only valid for a given motion planning problem. 
\end{itemize}
...
\subsubsection{Note: about Asumption~(\ref{eq:asumption-1})}%
...
closer and closer (see (Figure~\ref{fig:convergence-diagram}) as an example). And 
...
would not vanish at $t=1$. Therefore, Asumption~(\ref{eq:asumption-1}) is verified.
