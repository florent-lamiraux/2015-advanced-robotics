...
still very popular~\cite{Sekhavat-Svestka1998,HauserFastSmooth}. However, RS requires fine tuning of the termination condition (see 
...
algorithm or partial random shortcut (PRS) methods~\cite{Geraerts04clearancebased} that shortcut only one or a group of DOF.
...
which can address the problem of ~\ref{decoupled_DOF_optimization} but has longer 
computation time and is not using the information returned by the checker of which 
links are colliding to guide which DOF to choose for the PRS.
...
$\mathbf{u}=\po_2-\po_1$ which will be used as the orthogonal direction the bodies can not move along at the path parameter $\tcolli$.
...
We denote by $F$ the mapping from $\CS^{wp}$ to $\real^3$ defined by
\begin{equation}\label{eq:constraint}
F (\xx) =  R^T (\translation^{*} - \translation)
\end{equation}
...
Figure~\ref{2D_long} illustrates a \textit{very long} path example that RS or PRS will not manage to 
...
the extinction of the mobile base motion in the gradient-based optimized path is 
advanced in the right column of Table~\ref{tab:results}. Thus the problem addressed 
...
\begin{figure}[t]
	...
	\caption{Trajectories of wrists, end-effectors or center allong different paths: the initial path is represented in red, the RS optimized path in blue and the GB optimized path ($\alpha_{init}=0.1$) in green. For the freeflyer puzzle (bottom right), points $\po_1$, $\po_2$ and vectors $\mathbf{u}$ for constraints computation are also shown in grey and red.}
	\label{trajectories}
\end{figure}
...
\scalebox{0.9}{
  ...
  \toprule
    \textbf{Problem} & Path obtained from & Computation time  & Path length & Cost & Base traveled distance\\
    \midrule
    \midrule
    & Initial &  & $14.3$ & $ $ &\\
    ...
    \textbf{PR2-crossing-} & GB & $24.8\,\text{s}$ & $1.90$ & $ $ & $10^{-7}\,\text{m}$\\
    \textbf{arms} & PRS & $41.1\,\text{s}$ & $ 4.73$ & $ $ & $0\,\text{m}$\\
    & RS & $31.5\,\text{s}$ & $1.60$ & $ $ & $2.70\,\text{m}$\\
    \midrule
    & Initial &  & $ $ & $ $ &\\
    \textbf{PR2-in-} & GB & $\,\text{s}$ & $ $ & $ $ &\\
    \textbf{kitchen} & PRS & $\,\text{s}$ & $ $ & $ $ &\\
    & RS & $\,\text{s}$ & $ $ & $ $ &\\
    \midrule
    & Initial &  & $ $ & $ $ &\\
