...
\jvol{30} \jnum{15} \jyear{2016} \jmonth{August}
...
randomly sample the system configuration space~\cite{ref-spaceconfig} in
order to find a collision-free path, linking the initial and final configurations. 
...
Previous work on path optimization has been conducted. CHOMP algorithm~\cite{RatliffChomp} optimizes an initial guess provided as
...
generation with local holonimic considerations. But this method remains only 
...
distance constraints. Unlike our method however, anti-collision constraints are 
...
interval $[0,T]$ into $\CS$: $\xx_i(0) = q_0$, $\xx_i(T) = q_{n+1}$ for all $i$. As illustrated in (Figure~\ref{optim_grad}), let 
...
f(\xx) - f(\xx_{i}) = 0.
\end {equation}
As this constraint is non-linear, it is linearized around $\xx_{i}$. Let $\conf_{k\,i}$ denote the waypoint $k$ along path $\xx_i$.
The constraint on the position at $t_{coll\,i}$ is a function that depends only
on two waypoints $\conf_{k\,i}$, $\conf_{k+1\,i}$, with $0\leq k\leq n$. There exists 
...
$$
This expression represents the velocity of point $\po_2$ in the reference frame of $\body_1$ projected onto $\mathbf{u}$ along the motion of the robot defined by the following trajectory:
...
reached for $t=1$.
This expression being equal to zero requires either
\begin {enumerate}
...
would not vanish. Therefore, Equation~(\ref{eq:asumption-1}) is verified.
...
be traveled through. Typically, we 
...
\begin{figure}[t]
	\centering
	\includegraphics[width=10cm]{local_box_optim.pdf} % MOVE ROBOT TO THE ZOOM !
...
on a problem very encountered in the motion planning literature: a freeflyer puzzle 
...
% baxter:
An problem involving a Baxter-like\footnote{A torso rotation was added.} robot 
...
door, is presented (Figure~\ref{fig:trajectories:kitchen2}) with the mobile 
...
each example in the (Table~\ref{tab:results}). In some cases $\alpha_{init}$ was 
...
constraints of GB is typically important and $\alpha_{init}$ reduced.
...
easily gives up in some problems, such as the 2D slidding-robot, the 
...
\scalebox{0.9}{
  ...
  \toprule
    ...
    \midrule
    & Initial &  & $18.8$  &\\
    \textbf{Freeflyer-} & GB ($\alpha_{init}=0.05$) & $1.02\,\text{s}$ & $9.65$  &\\
...
time-competitive compared to random shortcut and proposes better quality paths, reducing the path length and removing unnecessary DOF motions. Our optimizer also manages to reduce a local detour in a long path while random shortcut methods will mostly fail.

In a future work, we plan to improve the computation speed of our method.
