...
\Procedure{Optimize}{$\xx_0$}
	\State $\alpha \gets \alpha_{init}$; $\epsilon \gets 10^{-3}$
	\State $minReached \gets false$
	\State $validConstraints \gets true$
	\While{($\textbf{not}(noCollision \textbf{ and } minReached)$)}
		\State $\p = \texttt{computeIterate}()$
		\State $minReached = (||\p||<\epsilon \textbf{ or } \alpha=1)$\label{algo:minReached}
		\State $\xx_1 \gets \xx_0 + \alpha\,\p$
		\If{($\alpha \neq 1$)}
			%\State \texttt{solveConstraints($\xx_1$)}
			\If{(\textbf{not}(\texttt{solveConstraints($\xx_1$)}))} \label{algo:solveCons}
				\State $\alpha \gets \alpha*0.5$
				\State $validConstraints \gets false$\label{algo:solveCons2}
			\Else
				\State $validConstraints \gets true$
			\EndIf
		\EndIf
		\If{(\textbf{not}(\texttt{validatePath($\xx_1$)}))}
			...
			\If{$(\alpha \neq 1)$}
				\State \texttt{addCollisionConstraints}()\label{algo:addCons}
				\State $\alpha \gets 1$
			\Else
				\If{($validConstraints$)}\label{algo:towardMin}
					\State $\alpha \gets \alpha_{init}$\label{algo:towardMin2}
				\EndIf
			\EndIf
		\Else
			\State $\xx_0 \gets \xx_1$
			\State $noCollision \gets true$\label{algo:noCollision}
			\State $\alpha \gets 0.5*(1+\alpha)$
		\EndIf
	\EndWhile
	\Return $\xx_0$
\EndProcedure
...
the solution of the current QP is tested (i.e. $\alpha = 1$). If this path is collision-free, the algorithm returns it as the solution (lines~\ref{algo:minReached}-\ref{algo:noCollision}). Otherwise, 
smaller steps are iteratively applied and tested toward the minimum (lines~\ref{algo:towardMin}-\ref{algo:towardMin2}), and constraints are added each time a collision is found (line~\ref{algo:addCons}).

If the constraint relinearization fails to validate Property~\ref{prop:collision free}, $\alpha$ is halved and the algorithm restarts from the latest valid path (lines~\ref{algo:solveCons}-\ref{algo:solveCons2}).
