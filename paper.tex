...
\textcolor{red}{We denote by
  \begin{itemize}
  \item $\po_1$ (resp. $\po_2$) the point belonging to body $\body_1$ (resp. $\body_2$) that coincides with $\po_c$ in configuration $\xx_{i+1}(\tcolli)$,
  ...
  \end{itemize}
  The position of $\po_1$ (resp. $\po_2$) in the global reference frame depends on the configuration $\conf$ of the robot. We therefore denote $\po_1(\conf)$ (resp. $\po_2(\conf)$) this position.
We define $\U$ as the unit vector attached to $\body_1$, linking points $\po_1$ and $\po_2$ in configuration $\xx_{i}(\tcolli)$ (Figure~\ref{contact-points} right):
$$
\U (\conf) = \rotation_1(\conf)\rotation_1^{T}(\xx_{i}(\tcolli))\frac{\po_2(\xx_{i}(\tcolli)) - \po_1(\xx_{i}(\tcolli))}{\|\po_2(\xx_{i}(\tcolli)) - \po_1(\xx_{i}(\tcolli))\|}
$$
Note that
\begin{itemize}
\item $\U$ depends on the configuration of the robot $\conf$, since $\U$ moves with $\body_1$,
\item $\U$ is well defined since configuration $\xx_{i}(\tcolli)$ is collision-free.
\end{itemize}
}
\textcolor{blue}{
Let $g$ be the function mapping the projection of vector $\overrightarrow{\po_1\,\po_2}$ on $\mathbf{u}$ to a configuration $\conf$ . For any $\conf \in \CS$:
$$
g (\conf) = \left(\po_2(\conf) - \po_1(\conf) | \mathbf{u}\right)
$$
}
...
f (\xx) = g(\xx (\tcolli))
$$
The constraint defined for any path $\xx$ by:
\begin {equation}\label{eq:new-constraint}
f(\xx) - f(\xx_{i}) = 0.
\end {equation}
aims at keeping colliding points $P_1$ and $P_2$ in configuration $\xx_{i+1} (\tcolli)$ apart in direction $\U$ defined by those points in configuration $\xx_{i} (\tcolli)$.
...
motions in the hole. 
The GB optimizer manages to modify the puzzle trajectory even in the hole, but remains less efficient than RS and PRS. We explain this behavior by the fact that, ........
...
launches are given. It is not a surprise that GB is globally slower than RS which converges faster. 
}
...
\scalebox{0.9}{
  ...
  \textbf{Freeflyer-puzzle} & ($\alpha_{init}=0.1$) & 0 ms & 0 & 0 & 0 \\
  \textbf{Double-arm 4-DOF} & & 39.6 ms & 46.8 & 55.5 & 63.5 \\
  \textbf{UR5-with-spheres} & (RRT, $\alpha_{init}=0.3$) & 3.18 s & 53.8 & 47.5 & 67.8 \\
  \textbf{UR5-industrial-example} & (RRT) & 0 ms & 0 & 0 & 0 \\
  \textbf{Baxter-in-office} & & 0 ms & 0 & 0 & 0 \\
  \textbf{PR2-crossing-arms} & & 882 ms & 19.9 & 43.2 & 95.2 \\
  \textbf{PR2-in-kitchen-1} & & 0 ms & 0 & 0 & 0 \\
  \bottomrule
  \end{tabular}  }%scalebox
\caption{Average results for 50 runs of several examples presented in the 
...
Except the low-DOF problem of the puzzle, our method almost always provides the 
shortest paths, whereas it is not even the best criterion to quantify the parasite 
motions disappearance.
TO COMPLETE!
%Finally, the complete vanishing of the mobile base motion in the GB optimized path is noted. 
%Thus the issue addressed in Figure~\ref{decoupled_DOF_optimization} is solved considering this DOF.
...
TODO !!
Contrary to a first idea that $\alpha_{init}$ is reduced to 0.05 to slightly modify the very constrained trajectory at each iteration.
...
\begin{figure}[H]
  ...
  \caption{Influence of $\alpha_{init}$ on the convergence graphs of the GB optimizer.}
...
\begin{figure}
  ...
  \caption{Other trajectories comparisons of end-effectors or mobile bases (initial path in red, RS output in blue and GB output in green).}
