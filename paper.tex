...
\textcolor{blue}{The constraint function is noted $f$}. Let us assume that at iteration $i$, $j$ constraints have been inserted before the current iteration. 
...
between frames of $\body_1$ (resp. $\body_2$) and the global frame.
...
\begin{figure}
	...
	and $\kappa$ resp. collision-free path, in-collision path, iteration step 
...
\textcolor{blue}{PARTIR A FINIR!!}
\textcolor{blue}{Remind that, the QP optimal step $\p$ computed by \texttt{computeIterate} is known.
The idea of the algorithm is to process ....
So the difficulty here is to handle 
...
Choosing to travel small steps from a valid path decrease the chances of being in collision. Besides if one occurs, the collision-constraint is computed on the last valid path.
$\alpha_{init}$ could be computed from geometrical considerations.
...
smaller steps toward the minimum are iteratively applied and tested, and constraints are added each time a collision is found with a reduced step (i.e. $\alpha \neq 1$).}

The collision detection on a path is handled by \texttt{validatePath}, which returns $true$ if the given path is collision-free.
